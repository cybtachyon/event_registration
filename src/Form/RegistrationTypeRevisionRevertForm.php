...
/**
 * Provides a form for reverting a Registration type revision.
 *
 * @ingroup event_registration
 */
class RegistrationTypeRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'event_registration_type_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Registration type: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Registration type %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\event_registration\Entity\RegistrationTypeInterface $revision
   ...
   */
  protected function prepareRevertedRevision(RegistrationTypeInterface $revision, FormStateInterface $form_state) {
